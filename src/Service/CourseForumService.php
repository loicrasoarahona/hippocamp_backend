...
class CourseForumService
{
    ...
    public function getNbReplies(CourseForum $forum)
    {
        $retour = $this->em->getRepository(CourseForumReply::class)->createQueryBuilder('reply')
            ->select('count(reply.id)')
            ->where('reply.forum=:forumId')
    ...
    public function getDisplayName(CourseForum $forum)
    {
        $user = $forum->getMUser();
